(ns spelunky.codecs
  ...
  (:use [gloss.core :only [defcodec ordered-map compile-frame repeated]]
        ...
        [spelunky.codecs.internal :only [hex-string-le variable-length-integer buffer-store timestamp script]]))
...
(defcodec input
  (ordered-map
   ...
   :script (script (repeated :ubyte :prefix variable-length-integer))   ; https://en.bitcoin.it/wiki/Script
...
(defcodec output
  (ordered-map
   ...
   :script (script (repeated :ubyte :prefix variable-length-integer)))) ; see input script
...
(defcodec txn
  (buffer-store
   ...
   (fn [val buf] (assoc val :hash (bitcoin-hash buf)))))                ; this transaction's hash
...
(defcodec block-header
  (buffer-store
   ...
   (fn [val] (dissoc val :hash))
   (fn [val buf] (assoc val :hash (bitcoin-hash buf)))))                ; this block's hash
